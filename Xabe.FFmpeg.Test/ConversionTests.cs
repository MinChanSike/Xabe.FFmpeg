...
namespace Xabe.FFmpeg.Test
{
    public class ConversionTests
    {
        ...
        [Theory]
        [InlineData(12000000)]
        public async Task SetBitrateTest(int bitrate)
        {
            string outputPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + FileExtensions.Mp4);

            IMediaInfo info = await MediaInfo.Get(Resources.Mp4);
            ...
            var result = await MediaInfo.Get(outputPath);

            Assert.Equal(1, result.VideoStreams.Count());
            Assert.Equal(bitrate, result.VideoStreams.First().Bitrate);
        }
    }
}
